...
```bash
aws cloudformation delete-stack --stack-name rest-dynamodb
```
...
To create an item, execute the following command from a terminal prompt:

```bash
curl -v -d '{"id": "1","name":"Joseph"}' -H 'Content-Type: application/json' [endpointUrl]

### for example ###

curl -v -d '{"id": "1","name":"Joseph"}' -H 'Content-Type: application/json' https://qp26mtw3te.execute-api.us-east-1.amazonaws.com/Prod/
```

> **NOTE:** The curl command uses HTTP method `POST` by default when request body data is included.

### List all Items

To list all items, execute the following command at a terminal prompt:
...
curl -v https://qp26mtw3te.execute-api.us-east-1.amazonaws.com/Prod/
```

### Find an Item

To find a single item, execute the following command at a terminal prompt:

```bash
curl -v [endpointUrl]/[itemId]

### for example ###

curl -v https://qp26mtw3te.execute-api.us-east-1.amazonaws.com/Prod/1
```
